...
AgentBarShape is a level 7 CogAlpha domain agent in the Geometric & Fusion layer. Its paper focus is: Candlestick body, shadow, symmetry, and shape descriptors. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentBarShape: Candlestick body, shadow, symmetry, and shape descriptors.
...
5. Preserve lineage with agent_skill: alpha-bar-shape, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentBarShape, the primary objective is to express candlestick body, shadow, symmetry, and shape descriptors. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
- Confirm rationale names the Geometric & Fusion mechanism and not just implementation syntax.
- Confirm output is strict JSON compatible with AlphaCandidateBatch.

## Trace Expectations

Runtime traces should be able to record skill_name: alpha-bar-shape, paper_agent_name: AgentBarShape, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
