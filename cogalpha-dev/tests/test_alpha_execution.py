...
def test_runtime_guard_ignores_structural_nan_outside_membership():
...
'''
    ...
    result = execute_alpha_candidate(make_candidate(code), panel)
    assert result.isna().to_numpy().mean() == 0.5

    report = run_runtime_alpha_code_guard(make_candidate(code), panel)

    assert report.status == GuardStatus.PASS
    assert report.metadata["evaluated_count"] == 4
    assert report.metadata["nan_fraction"] == 0.0
