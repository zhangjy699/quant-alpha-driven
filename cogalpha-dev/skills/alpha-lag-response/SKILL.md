...
AgentLagResponse is a level 4 CogAlpha domain agent in the Price-Volatility Behavior layer. Its paper focus is: Delayed adjustment between volatility, volume, and returns. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentLagResponse: Delayed adjustment between volatility, volume, and returns.
...
5. Preserve lineage with agent_skill: alpha-lag-response, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentLagResponse, the primary objective is to express delayed adjustment between volatility, volume, and returns. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
Runtime traces should be able to record skill_name: alpha-lag-response, paper_agent_name: AgentLagResponse, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
