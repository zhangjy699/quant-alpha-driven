...
AgentRegimeGating is a level 6 CogAlpha domain agent in the Stability & Regime-Gating layer. Its paper focus is: Adaptive gates based on volatility, trend, or liquidity states. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentRegimeGating: Adaptive gates based on volatility, trend, or liquidity states.
...
5. Preserve lineage with agent_skill: alpha-regime-gating, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentRegimeGating, the primary objective is to express adaptive gates based on volatility, trend, or liquidity states. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
Runtime traces should be able to record skill_name: alpha-regime-gating, paper_agent_name: AgentRegimeGating, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
