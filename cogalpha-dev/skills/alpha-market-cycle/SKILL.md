...
AgentMarketCycle is a level 1 CogAlpha domain agent in the Market Structure & Cycle layer. Its paper focus is: Long-term trends, market phases, and cyclical state transitions. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentMarketCycle: Long-term trends, market phases, and cyclical state transitions.
...
5. Preserve lineage with agent_skill: alpha-market-cycle, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentMarketCycle, the primary objective is to express long-term trends, market phases, and cyclical state transitions. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
Runtime traces should be able to record skill_name: alpha-market-cycle, paper_agent_name: AgentMarketCycle, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
