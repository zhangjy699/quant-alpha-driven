...
AgentLiquidity is a level 3 CogAlpha domain agent in the Price-Volume Dynamics layer. Its paper focus is: Liquidity, price impact, and volume-adjusted movement. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentLiquidity: Liquidity, price impact, and volume-adjusted movement.
...
5. Preserve lineage with agent_skill: alpha-liquidity, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentLiquidity, the primary objective is to express liquidity, price impact, and volume-adjusted movement. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
Runtime traces should be able to record skill_name: alpha-liquidity, paper_agent_name: AgentLiquidity, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
