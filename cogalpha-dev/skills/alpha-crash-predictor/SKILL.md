...
AgentCrashPredictor is a level 2 CogAlpha domain agent in the Extreme Risk & Fragility layer. Its paper focus is: Crash precursors and regime breakdown signals. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentCrashPredictor: Crash precursors and regime breakdown signals.
...
5. Preserve lineage with agent_skill: alpha-crash-predictor, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentCrashPredictor, the primary objective is to express crash precursors and regime breakdown signals. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
- Confirm rationale names the Extreme Risk & Fragility mechanism and not just implementation syntax.
- Confirm output is strict JSON compatible with AlphaCandidateBatch.

## Trace Expectations

Runtime traces should be able to record skill_name: alpha-crash-predictor, paper_agent_name: AgentCrashPredictor, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
