...
AgentStability is a level 6 CogAlpha domain agent in the Stability & Regime-Gating layer. Its paper focus is: Temporal consistency, persistence, and smoothness. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentStability: Temporal consistency, persistence, and smoothness.
...
5. Preserve lineage with agent_skill: alpha-stability, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentStability, the primary objective is to express temporal consistency, persistence, and smoothness. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
- Confirm rationale names the Stability & Regime-Gating mechanism and not just implementation syntax.
- Confirm output is strict JSON compatible with AlphaCandidateBatch.

## Trace Expectations

Runtime traces should be able to record skill_name: alpha-stability, paper_agent_name: AgentStability, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
