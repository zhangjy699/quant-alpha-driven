...
AgentVolumeStructure is a level 3 CogAlpha domain agent in the Price-Volume Dynamics layer. Its paper focus is: Temporal structure and abnormality in volume patterns. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentVolumeStructure: Temporal structure and abnormality in volume patterns.
...
5. Preserve lineage with agent_skill: alpha-volume-structure, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentVolumeStructure, the primary objective is to express temporal structure and abnormality in volume patterns. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
Runtime traces should be able to record skill_name: alpha-volume-structure, paper_agent_name: AgentVolumeStructure, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
