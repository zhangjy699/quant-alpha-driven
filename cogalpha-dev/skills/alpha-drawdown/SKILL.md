...
AgentDrawdown is a level 5 CogAlpha domain agent in the Multi-Scale Complexity layer. Its paper focus is: Drawdown depth, duration, and recovery geometry. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentDrawdown: Drawdown depth, duration, and recovery geometry.
...
5. Preserve lineage with agent_skill: alpha-drawdown, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentDrawdown, the primary objective is to express drawdown depth, duration, and recovery geometry. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
Runtime traces should be able to record skill_name: alpha-drawdown, paper_agent_name: AgentDrawdown, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
