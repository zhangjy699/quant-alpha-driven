...
AgentFractal is a level 5 CogAlpha domain agent in the Multi-Scale Complexity layer. Its paper focus is: Multi-scale roughness and long-memory characteristics. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentFractal: Multi-scale roughness and long-memory characteristics.
...
5. Preserve lineage with agent_skill: alpha-fractal, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentFractal, the primary objective is to express multi-scale roughness and long-memory characteristics. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
- Confirm rationale names the Multi-Scale Complexity mechanism and not just implementation syntax.
- Confirm output is strict JSON compatible with AlphaCandidateBatch.

## Trace Expectations

Runtime traces should be able to record skill_name: alpha-fractal, paper_agent_name: AgentFractal, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
