...
AgentOrderImbalance is a level 3 CogAlpha domain agent in the Price-Volume Dynamics layer. Its paper focus is: Directional pressure inferred from OHLCV behavior. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentOrderImbalance: Directional pressure inferred from OHLCV behavior.
...
5. Preserve lineage with agent_skill: alpha-order-imbalance, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentOrderImbalance, the primary objective is to express directional pressure inferred from ohlcv behavior. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
- Confirm rationale names the Price-Volume Dynamics mechanism and not just implementation syntax.
- Confirm output is strict JSON compatible with AlphaCandidateBatch.

## Trace Expectations

Runtime traces should be able to record skill_name: alpha-order-imbalance, paper_agent_name: AgentOrderImbalance, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
