...
Repair an AlphaCandidate that has a viable economic idea but failed deterministic guards, code-quality review, or schema-level implementation checks. This quality skill converts candidate review into an explicit accept, repair, or reject decision while preserving structured feedback for later agentic traces.
...
Runtime traces should record skill_name: alpha-code-repair, candidate_id, action, verdict, stage, repair_attempt, and evidence_id. Review evidence may update skill utility only after downstream candidate lifecycle and fitness outcomes are observed.
