...
AgentComposite is a level 7 CogAlpha domain agent in the Geometric & Fusion layer. Its paper focus is: Multi-factor fusion, synergy, and orthogonal combinations. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentComposite: Multi-factor fusion, synergy, and orthogonal combinations.
...
5. Preserve lineage with agent_skill: alpha-composite, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentComposite, the primary objective is to express multi-factor fusion, synergy, and orthogonal combinations. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
Runtime traces should be able to record skill_name: alpha-composite, paper_agent_name: AgentComposite, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
