...
AgentPriceVolumeCoherence is a level 3 CogAlpha domain agent in the Price-Volume Dynamics layer. Its paper focus is: Coherence between price movement and volume behavior. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentPriceVolumeCoherence: Coherence between price movement and volume behavior.
...
5. Preserve lineage with agent_skill: alpha-price-volume-coherence, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentPriceVolumeCoherence, the primary objective is to express coherence between price movement and volume behavior. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
Runtime traces should be able to record skill_name: alpha-price-volume-coherence, paper_agent_name: AgentPriceVolumeCoherence, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
