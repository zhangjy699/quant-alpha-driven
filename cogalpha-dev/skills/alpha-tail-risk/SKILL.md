...
AgentTailRisk is a level 2 CogAlpha domain agent in the Extreme Risk & Fragility layer. Its paper focus is: Tail-risk exposure and stress accumulation. Generate compact OHLCV alpha candidates whose mechanism is specific to this layer rather than a generic moving-average or stacked-indicator pattern.
...
2. Plan one financial mechanism aligned to AgentTailRisk: Tail-risk exposure and stress accumulation.
...
5. Preserve lineage with agent_skill: alpha-tail-risk, generation, and guidance mode so later trace review can attribute outcomes.
...
Target balanced improvement across IC, RankIC, ICIR, RankICIR, and MI. For AgentTailRisk, the primary objective is to express tail-risk exposure and stress accumulation. in a way that can survive both qualified and elite minima. Treat single-metric gains as diagnostic until the full gate clears.
...
Runtime traces should be able to record skill_name: alpha-tail-risk, paper_agent_name: AgentTailRisk, candidate_id, generation, guidance_mode, stage, and evidence_id. Candidate metadata should make utility updates trace-grounded and should not imply prompt promotion without review.
